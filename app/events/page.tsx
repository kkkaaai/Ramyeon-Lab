...
export default async function EventsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-pixel text-rl-yellow text-xl uppercase">Upcoming Sessions</h1>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="font-pixel text-rl-yellow text-sm">
                ...
                {s.is_active && me && (
                  <Link
                    href="/queue/join"
                    className="font-sans text-xs uppercase tracking-[1px] bg-rl-yellow text-rl-base px-4 py-2 border-2 border-rl-border shadow-pixel rounded"
                  >
                    JOIN THE QUEUE →
                  </Link>
                )}
      ...
      {past.length > 0 && (
        <details className="mt-8">
          ...
          <div className="mt-4 space-y-2">
            {past.map((s) => (
              <div key={s.id} className="bg-rl-surface border border-rl-muted/40 rounded p-3 font-sans text-sm text-rl-muted">
